...
## 7) Сборка и тесты: только Maven

- Для этого репозитория использовать только инструментарий Maven: `mvn` и `./mvnw`.
- Команды Gradle (`gradle`, `./gradlew`) не использовать: проект собирается и тестируется исключительно через Maven.
- Базовая команда проверки перед фиксацией изменений: `./mvnw -q -Dmaven.site.skip=true test`.

## 8) Политика legacy в prerelease

- Проект находится в prerelease-стадии: **legacy-совместимость и legacy-копии файлов не поддерживаются**, если это явно не запрошено отдельной задачей.
- Канонический файл агентских инструкций в репозитории: только `AGENTS.md` (без дублирующих копий вроде `Agents.md`).

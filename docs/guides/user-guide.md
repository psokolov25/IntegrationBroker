...
## Best practices: correlation и idempotency

- Всегда передавайте `X-Correlation-Id` и `X-Request-Id` во входящих и исходящих вызовах.
- Для write-операций внешних систем передавайте `Idempotency-Key`.
- Для replay ориентируйтесь на связку: `correlationId` -> запись в idempotency -> DLQ/outbox.
- При повторяющихся 429/5xx сначала проверяйте retriable-ошибки и только затем запускайте массовый replay.

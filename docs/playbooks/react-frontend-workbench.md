...
### 2.9. Локализация интерфейса (обязательно)

- Интерфейс frontend должен быть **в первую очередь на русском языке** (RU-first).
- Архитектура UI должна поддерживать расширение на другие языки без переписывания компонентов.
- Вторым обязательным языком является **английский** (EN-second).
- Все пользовательские тексты выносятся в словари локализации (`ru-RU`, `en-EN`) и не хардкодятся в компонентах.
- Базовые локали должны использовать региональные коды (`ru-RU`, `en-EN`) с fallback-механизмом по языку.
...
- использовать разнообразные типы полей по типу данных для лучшей эргономики:
  - текстовые/числовые поля для строк и чисел,
  - поля с маской для телефонов, документов и иных форматированных значений,
  - одиночный выбор (`select`, radio) для фиксированных enum,
  - мультивыбор (`multi-select`, chips, checkbox-group) для списков значений,
  - календарь для дат, date-time picker для дат со временем,
  - time picker для времени,
  - переключатели (`toggle`, checkbox) для булевых флагов,
  - textarea/editor для длинных структурированных значений (JSON/Groovy) с подсветкой/валидацией.

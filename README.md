...
[![Micronaut](https://img.shields.io/badge/Micronaut-4.x-00bcd4)](#)
[![Build-Maven](https://img.shields.io/badge/build-maven-informational)](#)
[![Docs-Markdown--first](https://img.shields.io/badge/docs-markdown--first-success)](#)
...
- [5. Примеры payload и сценариев](#5-примеры-payload-и-сценариев)
- [6. Документация](#6-документация)
- [7. Конфигурация и окружение](#7-конфигурация-и-окружение)
- [8. Безопасность и эксплуатация](#8-безопасность-и-эксплуатация)
- [9. Frontend Workbench (старт реализации)](#9-frontend-workbench-старт-реализации)
- [10. Лицензирование и вклад](#10-лицензирование-и-вклад)
...
```bash
./mvnw -q -Dmaven.site.skip=true test
```
...
- `docs/playbooks/readme-standards.md`
...
- `docs/roadmap/implementation-backlog.md` — текущий пакет задач (пакеты 01/02).
- `docs/roadmap/release-notes-packet-02.md` — релиз-ноты по пакету 02.
- `docs/roadmap/release-notes-packet-03.md` — релиз-ноты по пакету 03.

### 6.4. Экспорт документации из Markdown
...
- Важно: экспорт строится из Markdown (bundle/manifest/HTML), бинарники в git не добавляются.
...
- Базовая конфигурация: `src/main/resources/application.yml`.
- Тестовый профиль: `src/test/resources/application-test.yml`.
- Примеры runtime-конфигурации и payload: `src/main/resources/examples/`.
- Перед изменениями контрактов VM/DB сверяйтесь с `AGENTS.md` и источниками в `/workspace/VisitManager`, `/workspace/DataBus`.
...
## 9. Frontend Workbench (старт реализации)
...
- role-aware UI;
- RU-first локализация (`ru-RU`) + `en-EN` и расширение по популярности языков;
...
Текущий прототип расположен в `apps/workbench` (см. `apps/workbench/README.md`).

## 10. Лицензирование и вклад

Перед внесением изменений:

1. Сверяйтесь с `AGENTS.md` (единственный канонический файл инструкций; legacy-копии не ведём).
